...
#+end_src
** Add nix-installed packages to path
For whatever reason, doom does not pick up on my PATH has having nix.
...
#+end_src
*** Keymaps for windows
Add vim-style keybinds for window splitting and switching.
I use Colemak, so the letters mirror an arrow cluster where my right hand rests ("ijkl" on qwerty).
#+begin_src elisp
...
#+end_src
** Doom modeline
- Set the color of the filename when modified to catppuccin latte's maroon.
...
While using Catppuccin flavors, I use the "mantle" color code.
