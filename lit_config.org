...
#+end_src
This stopped working, so I'll set it to nil.
#+begin_src elisp
...
** Avy
Per tecosaur's configuration (which targets Colemak), this sets Avy's character prompts to letters in the Gallium/Graphite layouts' homerow:
#+begin_src elisp
(after! avy
...
In neovim I use ~leap.nvim~ mapped to "s" and "S".
The below keymaps map ~avy-goto-char-2-below~ and its ~above~ variant to the same keys.
Note that ~avy-goto-char-2~ matches two characters both above and below the cursor.
#+begin_src elisp
...
*** List all nodes without back
Running ~bk-find-unlinked-org-roam-nodes~ will list links to the nodes without any links in whatever buffer that the cursor is currently located.

Source: user nonhok on reddit https://www.reddit.com/r/OrgRoam/comments/16wgche/finding_all_orgroam_nodes_without_a_link_and/.
#+begin_src emacs-lisp
